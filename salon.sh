...
# show available services
  ...
  # if the choice is not a number
    ...
    then
    # send to main menu
      MAIN_MENU "Sorry, that is not a valid number! Please enter a valid number."
    else
      ...
      # get customer phone number
        ...
        read CUSTOMER_PHONE
        # check if is a new customer or not by phone number
        CUSTOMER_NAME=$($PSQL "SELECT name FROM customers WHERE phone = '$CUSTOMER_PHONE'")
        # if is a new customer
          if [[ -z $CUSTOMER_NAME ]]
          then
          # get the name, phone and insert it to the table with the selected service
